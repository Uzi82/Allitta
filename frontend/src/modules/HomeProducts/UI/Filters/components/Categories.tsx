...
import { RadioGroupStyled, RadioGroupHeader, RadioGroupExpand, RadioGroupList } from './styled';
...
export const Categories: React.FC<CategoriesProps> = () => {
  ...
  const [currValues, setCurrValues] = React.useState(initialState);

  const [isOpen, setIsOpen] = React.useState(true)

  React.useEffect(() => {
    const id = setTimeout(() => {
      dispatch(setCategories(currValues))
    }, 500)
  ...
  return (
    <RadioGroupStyled>
      <RadioGroupHeader>
        ...
        <RadioGroupExpand $isOpen={isOpen} onClick={() => setIsOpen(p => !p)}>
      ...
      <RadioGroupList $isOpen={isOpen}>
        {
          data.map((category) => (
            <CustomCheckbox
              key={category}
              checked={currValues[category]}
              onChange={() => {
